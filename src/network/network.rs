...
use crate::{db::migrations::
    networks::{
        ...
        select_queries::{GET_ALL_NETWORKS, GET_NETWORK, GET_NETWORK_ASSETS}
    }, 
    error::StabuseError, types::{self, types::{Asset, NetworkDB}}, utils::{utils::{hashmap_to_json_value, transform_assets_to_uppercase}, validation::validate_assets}};
...
pub async fn add_asset_to_network(pool: &PgPool, chain_id:i32, asset: HashMap<String, String>) -> Result<(), StabuseError> {
...
pub async fn get_network_supported_assets(pool: &PgPool, chain_id: i64) -> Result<Asset, StabuseError> {
    let result: (Value,) = sqlx::query_as(GET_NETWORK_ASSETS)
        .bind(chain_id)
        .fetch_one(pool)
        .await?;

    let assets: Asset = serde_json::from_value(result.0)
        .map_err(|e| StabuseError::SerdeError(e.to_string()))?;
...
pub async fn get_network(pool: &PgPool, chain_id: i64) -> Result<NetworkDB, StabuseError> {
    ...
    let supported_assets: HashMap<String, String> = match row.try_get::<Value, _>("supported_assets")? {
        Value::Object(map) => map
            ...
            .collect::<Result<_, _>>().expect("Error"),
        _ => return Err(StabuseError::SerdeError("Invalid JSON format for supported_assets".into())),
    };
...
pub async fn get_all_networks(pool: &PgPool) -> Result<Vec<NetworkDB>, StabuseError> {
    let rows = sqlx::query(GET_ALL_NETWORKS)
        .fetch_all(pool)
        .await?;
